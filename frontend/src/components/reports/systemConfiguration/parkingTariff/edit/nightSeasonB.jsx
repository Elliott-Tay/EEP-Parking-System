...
export default function TariffSetupNightSeasonB() {
  const navigate = useNavigate();
      ...
      useEffect(() => {
        const fetchTariff = async () => {
          try {
            ...
            const response = await fetch(`${process.env.REACT_APP_BACKEND_API_URL}/api/tariff/tariff-rates?rateType=Night Season(B)`, {
            ...
            allRates.forEach(slot => {
              const mapped = {
                ...
                rate_type: slot.rate_type || dynamicRateTypes[0] || "Night Season(B)",
                every: slot.every || 60,
                ...
                contractClass: slot.rate_type || dynamicRateTypes[0] || "Night Season(B)",
              };
      ...
      const addCarTimeSlot = (day, vehicleType = "Car/HGV", rateType = "Night Season(B)") => {
        const newSlot = {
          vehicleType,
          from: "08:00",
          to: "18:00",
      ...
      const addMCycleTimeSlot = (day, vehicleType = "MC", rateType = "Night Season(B)") => {
        const newSlot = {
          vehicleType,
          from: "08:00",
          to: "18:00",
      ...
      const addAllTimeSlot = (day, vehicleType = "Car/HGV/MC", rateType = "Night Season(B)") => {
        const newSlot = {
          vehicleType,
          from: "08:00",
          to: "18:00",
      ...
      const handleSaveDay = async (day) => {
        ...
        const payloadSlots = slots.map(slot => ({
          ...
          rateType: slot.rate_type || slot.rateType || "Night Season(B)",
      ...
      return (
        <div className="min-h-screen bg-gray-50 p-8">
          <h1 className="text-2xl font-bold mb-4">Tariff Setup for Night Season(B) Rates</h1>
