...
export default function TariffSetupDaySeasonB() {
  ...
  // Fetch existing rates from backend
  useEffect(() => {
    const fetchRates = async () => {
      try {
        const token = localStorage.getItem("token");
        const res = await fetch(
          `${process.env.REACT_APP_BACKEND_API_URL}/api/tariff/tariff-setup?vehicleType=DaySeasonB`,
          {
            headers: {
        ...
        daysOfWeek.forEach((day) => {
          newRates[day] =
            data[day] && data[day].length ? data[day] : [{ ...initialSlot }];
        });
        setRates(newRates);
      } catch (err) {
        console.error(err);
  ...
  const removeTimeSlot = (day, index) => {
    setRates((prev) => {
      ...
      return {
        ...prev,
        [day]: updatedDay.length ? updatedDay : [{ ...initialSlot }],
      };
    });
  };
  ...
  const isOverlapping = (slots) => {
    for (let i = 0; i < slots.length; i++) {
      ...
      for (let j = i + 1; j < slots.length; j++) {
        ...
        if (fromI && toI && fromJ && toJ && fromI < toJ && toI > fromJ)
          return true;
      }
    }
    return false;
  ...
  const handleSave = async () => {
    ...
    for (const [day, slots] of Object.entries(rates)) {
      sanitizedRates[day] = slots.map((slot) => {
        ...
        newSlot.from = DateTime.fromFormat(slot.from, "HH:mm", {
          zone: "Asia/Singapore",
        }).toISOTime({ suppressMilliseconds: true });
        newSlot.to = DateTime.fromFormat(slot.to, "HH:mm", {
          zone: "Asia/Singapore",
        }).toISOTime({ suppressMilliseconds: true });
        return newSlot;
      });
    ...
    const payload = {
      vehicleType: "DaySeasonB",
      effectiveStart: effectiveStart
        ? effectiveStart + "T00:00:00+08:00"
        : null,
    ...
    try {
      ...
      const data = await res.json();
      if (res.ok) alert("Day Season(B) rates saved successfully!");
  ...
  return (
    <div className="min-h-screen bg-gray-50 p-8">
      <h1 className="text-2xl font-bold mb-4">Tariff Setup for Day Season(B)</h1>
                    ...
                    {Object.entries(slot).map(([field, value]) => (
                      <td key={field} className="border px-2 py-1">
                        <input
                          ...
                          value={value}
                          onChange={(e) =>
                            handleInputChange(day, idx, field, e.target.value)
                          }
