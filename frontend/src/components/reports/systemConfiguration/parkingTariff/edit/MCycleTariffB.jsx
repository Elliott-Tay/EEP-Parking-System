...
export default function TariffSetupMotorcycleB() {
  const navigate = useNavigate();
  const daysOfWeek = ["Mon", "Tue", "Wed", "Thu", "Fri", "Sat", "Sun", "PH"];

  // Default slot to pre-fill each day
  const initialSlot = {
    from: "08:00",
    to: "18:00",
    rateType: "Hourly",
  ...
  const [rates, setRates] = useState(
    daysOfWeek.reduce((acc, day) => {
      acc[day] = [{ ...initialSlot }]; // Insert initial slot for CRUD
      return acc;
    }, {})
  );
  ...
  // Fetch existing Motorcycle rates (override initial slots if data exists)
  useEffect(() => {
    const fetchRates = async () => {
      try {
        ...
        daysOfWeek.forEach(day => {
          // If backend has data, use it; otherwise keep initial slot
  ...
  const removeTimeSlot = (day, index) => {
    setRates(prev => {
      ...
      // Ensure at least one slot remains for CRUD convenience
                  ...
                  <tr key={idx} className="text-center">
                    {Object.entries(slot).map(([field, value]) => (
                      <td key={field} className="border px-2 py-1">
                        <input type={field==="from"||field==="to"?"time":"text"}
                               value={value} 
