...
export default function TariffSetupClass1() {
  const navigate = useNavigate();
    ...
    useEffect(() => {
      const fetchTariff = async () => {
        try {
          ...
          const response = await fetch(`${process.env.REACT_APP_BACKEND_API_URL}/api/tariff/tariff-rates?rateType=Class1`, {
          ...
          allRates.forEach(slot => {
            const mapped = {
              ...
              rate_type: slot.rate_type || dynamicRateTypes[0] || 'Class1',
              every: slot.every || 60,
              ...
              contractClass: slot.rate_type || dynamicRateTypes[0] || "Class1",
            };
    ...
    const addCarTimeSlot = (day, vehicleType = "Car/HGV", rateType = "Class1" ) => {
      const newSlot = {
        vehicleType,
        from: "08:00",
        to: "18:00",
    ...
    const addMCycleTimeSlot = (day, vehicleType = "MC", rateType = "Class1") => {
      const newSlot = {
        vehicleType,
        from: "08:00",
        to: "18:00",
    ...
    const addAllTimeSlot = (day, vehicleType = "Car/HGV/MC", rateType = "Class1") => {
      const newSlot = {
        vehicleType,
        from: "08:00",
        to: "18:00",
    ...
    const handleSaveDay = async (day) => {
      ...
      const payloadSlots = slots.map(slot => ({
        ...
        rateType: slot.rate_type || slot.rateType || "Class1",
    ...
    return (
      <div className="min-h-screen bg-gray-50 p-8">
        <h1 className="text-2xl font-bold mb-4">Tariff Setup for Class 1 Rates</h1>
