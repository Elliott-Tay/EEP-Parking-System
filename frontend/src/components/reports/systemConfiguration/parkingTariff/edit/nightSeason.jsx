...
export default function TariffSetupNightSeason() {
  ...
  // Optional: fetch existing Night Season rates from backend
  useEffect(() => {
    const fetchRates = async () => {
      try {
        const res = await fetch(
          `${process.env.REACT_APP_BACKEND_API_URL}/api/tariff/tariff-setup?vehicleType=NightSeason`
        );
        ...
        console.error(err);
        alert("Failed to load existing Night Season data");
      }
    };
  ...
  const handleSave = async () => {
    ...
    for (const [day, slots] of Object.entries(rates)) {
      sanitizedRates[day] = slots.map(slot => {
        ...
        newSlot.from = DateTime.fromFormat(slot.from, "HH:mm", { zone: "Asia/Singapore" })
          .toISOTime({ suppressMilliseconds: true });
        newSlot.to = DateTime.fromFormat(slot.to, "HH:mm", { zone: "Asia/Singapore" })
          .toISOTime({ suppressMilliseconds: true });
        return newSlot;
      });
    }
    ...
    try {
      ...
      const data = await res.json();
      if (res.ok) alert("Night Season rates saved successfully!");
  ...
  return (
    <div className="min-h-screen bg-gray-50 p-8">
      <h1 className="text-2xl font-bold mb-4">Tariff Setup for Night Season</h1>
