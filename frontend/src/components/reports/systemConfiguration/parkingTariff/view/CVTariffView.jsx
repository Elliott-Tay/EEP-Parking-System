...
export default function TariffSetupCarVanView({ rates }) {
  const navigate = useNavigate();
  const daysOfWeek = ["Mon", "Tue", "Wed", "Thu", "Fri", "Sat", "Sun", "PH"];

  return (
    <div className="min-h-screen bg-gray-50 p-8">
      <h1 className="text-2xl font-bold mb-4">Tariff Setup for Car/Van (View Only)</h1>

      {/* Effective Date/Time */}
      <div className="grid grid-cols-2 gap-4 mb-6">
        <div>
          <label>Effective Start Date (dd/mm/yyyy)</label>
          <div className="border rounded p-2 w-full bg-gray-100">{rates?.effectiveStartDate || "-"}</div>
        </div>
        <div>
          <label>Effective Start Time (HH:MM)</label>
          <div className="border rounded p-2 w-full bg-gray-100">{rates?.effectiveStartTime || "-"}</div>
        </div>
        <div>
          <label>Effective End Date (dd/mm/yyyy)</label>
          <div className="border rounded p-2 w-full bg-gray-100">{rates?.effectiveEndDate || "-"}</div>
        </div>
        <div>
          <label>Effective End Time (HH:MM)</label>
          <div className="border rounded p-2 w-full bg-gray-100">{rates?.effectiveEndTime || "-"}</div>
        </div>
      </div>
