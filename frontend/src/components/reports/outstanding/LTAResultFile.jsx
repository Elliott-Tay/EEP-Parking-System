...
// Optional: Reusable badge for send status
function StatusBadge({ sendTime }) {
  return sendTime ? (
    <span className="inline-flex items-center gap-2 px-2.5 py-1 rounded-full text-xs font-medium border bg-green-100 text-green-800 border-green-300">
      <span className="w-2 h-2 rounded-full bg-green-500 animate-pulse"></span>
      Sent
    </span>
  ) : (
    <span className="inline-flex items-center gap-2 px-2.5 py-1 rounded-full text-xs font-medium border bg-yellow-100 text-yellow-800 border-yellow-300">
  ...
  const handleSearch = async () => {
    ...
    try {
      ...
      if (data && data.length > 0) {
        toast.success(`Found ${data.length} LTA result file${data.length !== 1 ? 's' : ''}`);
      } else {
        toast.info("No LTA result files found");
      }
    } catch (err) {
      console.error(err);
      setRecords([]);
      toast.error("Failed to fetch LTA result files");
    } finally {
      setLoading(false);
    }
  };
        ...
        <div className="max-w-7xl mx-auto">
          ...
          <div className="mb-6">
            <div className="flex items-center gap-3 mb-2">
              ...
              <div>
                <h1 className="text-3xl font-bold text-gray-900">Outstanding LTA Result File</h1>
                <p className="text-sm text-gray-600 mt-1">
                  View and manage LTA result files with settlement information
                </p>
              </div>
            </div>
          </div>
          ...
          <div className="bg-white/80 backdrop-blur-xl border border-white/20 rounded-xl shadow-lg mb-6">
            <div className="bg-gradient-to-r from-blue-50 to-blue-100/50 border-b px-6 py-4 rounded-t-xl">
              ...
              <p className="text-sm text-blue-700 mt-1">Filter LTA result files by settle date (optional)</p>
            </div>
              ...
              <div className="space-y-6">
                ...
                <div className="space-y-2">
                  ...
                  <p className="text-xs text-gray-500 flex items-center gap-1">
                    ...
                    Leave empty to view all LTA result files, or select a specific date to filter
                  </p>
                </div>
          ...
          {loading ? (
            <div className="bg-white/80 backdrop-blur-xl border border-white/20 rounded-xl shadow-lg p-12">
              <div className="flex flex-col items-center justify-center gap-4">
                ...
                <p className="text-gray-600 font-medium">Loading LTA result files...</p>
                ...
                <div className="bg-gradient-to-r from-green-50 to-green-100/50 border-b px-6 py-4 rounded-t-xl">
                  <div className="flex items-center justify-between">
                    <div className="flex items-center gap-2">
                      ...
                      <h3 className="font-semibold text-gray-900">LTA Result File Records</h3>
                    </div>
                  ...
                  <p className="text-sm text-green-700 mt-1">
                    {fileDate ? `LTA result files for ${fileDate}` : 'All LTA result files'}
                  </p>
                </div>
                  ...
                  <div className="hidden lg:block overflow-x-auto">
                    <table className="w-full">
                      ...
                      <tbody className="divide-y divide-gray-200">
                        {records.map((r, index) => (
                          <tr key={r.file_name + index} className="hover:bg-blue-50/50 transition-colors duration-150">
                              ...
                              {r.send_time ? (
                                <div className="space-y-1">
                                  <span className="text-sm text-gray-700 block">
                                    {new Date(r.send_time).toLocaleString()}
                                  </span>
                                  <StatusBadge sendTime={r.send_time} />
                                </div>
                              ) : (
                                <StatusBadge sendTime={r.send_time} />
                              )}
                      ...
                      >
                        <div className="space-y-3">
                          ...
                          <div className="grid grid-cols-2 gap-3 pt-3 border-t border-gray-200">
                            <div>
                              <span className="text-xs text-gray-600">Settle Date</span>
                              <p className="text-sm font-medium text-gray-900">
                                {r.settle_date ? new Date(r.settle_date).toLocaleDateString() : "-"}
                              </p>
                            </div>
                            <div>
                              <span className="text-xs text-gray-600">Send Time</span>
            ...
            <div className="bg-white/80 backdrop-blur-xl border border-white/20 rounded-xl shadow-lg">
              <div className="flex flex-col items-center justify-center p-12 text-center">
                ...
                <h3 className="font-semibold text-gray-900 mb-2">No LTA Result Files Found</h3>
                <p className="text-sm text-gray-600 max-w-md">
                  {fileDate 
                    ? `No LTA result files found for ${fileDate}. Try a different date or clear the filter.`
                    : "Click the Search button to load LTA result files."}
                </p>
              </div>
            </div>
          )}
          ...
          <div className="mt-6 bg-gradient-to-br from-blue-50 to-blue-100/50 border border-blue-200 rounded-xl p-5">
            <div className="flex items-start gap-3">
              ...
              <div>
                <h4 className="font-semibold text-blue-900 mb-2">LTA Result File Information</h4>
                <ul className="space-y-1.5 text-sm text-blue-800">
                  <li className="flex items-start gap-2">
                    <span className="w-1 h-1 rounded-full bg-blue-600 mt-2 flex-shrink-0"></span>
                    <span>LTA (Land Transport Authority) result files contain settlement outcomes for outstanding transactions</span>
                  </li>
                  <li className="flex items-start gap-2">
                    <span className="w-1 h-1 rounded-full bg-blue-600 mt-2 flex-shrink-0"></span>
                    <span>You can filter by settle date or view all result files by leaving the date field empty</span>
                  </li>
                  <li className="flex items-start gap-2">
                    <span className="w-1 h-1 rounded-full bg-blue-600 mt-2 flex-shrink-0"></span>
                    <span>Each file shows total transactions, total amount, settlement date, and send time</span>
                  </li>
                  <li className="flex items-start gap-2">
                    <span className="w-1 h-1 rounded-full bg-blue-600 mt-2 flex-shrink-0"></span>
                    <span>Files with a send time are marked as "Sent" (green), while others show "Pending" (yellow)</span>
                  </li>
                </ul>
              </div>
            </div>
