...
function OutstandingMovementTransaction() {
  ...
  const handleSearch = async () => {
    try {
      ...
      if (json.data && json.data.length > 0) {
        toast.success(`Found ${json.data.length} outstanding transaction${json.data.length !== 1 ? 's' : ''}`);
      } else {
        toast.info("No outstanding transactions found for the selected date");
      }
    } catch (err) {
      console.error(err);
  ...
  const getStatusInfo = (transaction) => {
    if (transaction.exit_trans_type) {
      return {
        label: "Exited",
        ...
        <div className="max-w-7xl mx-auto">
          ...
          <div className="mb-6">
            <div className="flex items-center gap-3 mb-2">
              ...
              <div>
                <h1 className="text-3xl font-bold text-gray-900">Outstanding Movement Transaction</h1>
                <p className="text-sm text-gray-600 mt-1">
                  View outstanding vehicle transactions and parking status by date
                </p>
              </div>
            </div>
          </div>
            ...
            <div className="bg-gradient-to-r from-blue-50 to-blue-100/50 border-b px-6 py-4 rounded-t-xl">
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-2">
                  ...
                  <h2 className="font-semibold text-gray-900">Report Parameters</h2>
                </div>
              </div>
              <p className="text-sm text-blue-700 mt-1">Select a report date to view outstanding transactions</p>
            </div>
              ...
              <div className="space-y-6">
                ...
                <div className="space-y-2">
                  ...
                  <p className="text-xs text-gray-500 flex items-center gap-1">
                    ...
                    Select the date to generate the outstanding movement transaction report
                  </p>
                </div>
          ...
          {loading ? (
            <div className="bg-white/80 backdrop-blur-xl border border-white/20 rounded-xl shadow-lg p-12">
              <div className="flex flex-col items-center justify-center gap-4">
                ...
                <p className="text-gray-600 font-medium">Searching outstanding transactions...</p>
              ...
              <div className="bg-gradient-to-r from-red-50 to-red-100/50 border-b px-6 py-4 rounded-t-xl">
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-2">
                    ...
                    <h3 className="font-semibold text-gray-900">Outstanding Transaction Records</h3>
                  </div>
                ...
                <p className="text-sm text-red-700 mt-1">Vehicle transactions for {reportDate}</p>
              </div>
                        ...
                        return (
                          <tr key={idx} className="hover:bg-blue-50/50 transition-colors duration-150">
                            <td className="px-4 py-3">
                              <span className="font-mono text-sm text-gray-900">{t.vehicle_number || "-"}</span>
                            </td>
                            <td className="px-4 py-3">
                              <span className="text-sm text-gray-700">{t.entry_trans_type || "-"}</span>
                            </td>
                            <td className="px-4 py-3">
                              <span className="text-sm text-gray-700">
                                {t.entry_datetime ? new Date(t.entry_datetime).toLocaleString() : "-"}
                              </span>
                            </td>
                            <td className="px-4 py-3">
                              <span className="text-sm text-gray-700">
                                {t.exit_datetime ? new Date(t.exit_datetime).toLocaleString() : "-"}
                              </span>
                            </td>
                            <td className="px-4 py-3">
                              <span className="text-sm font-medium text-gray-900">{t.parking_dur || "-"}</span>
                            </td>
                            <td className="px-4 py-3">
                              <span className="font-mono font-semibold text-sm text-gray-900">{t.parking_charges || "-"}</span>
                            </td>
                            <td className="px-4 py-3">
                              <span className="text-sm text-gray-700">{t.card_type || "-"}</span>
                            </td>
                            <td className="px-4 py-3">
                              <span className="font-mono text-sm text-gray-900">{t.card_number || "-"}</span>
                            </td>
                            <td className="px-4 py-3">
                      ...
                      >
                        <div className="space-y-3">
                          ...
                          <div className="flex items-center justify-between">
                            ...
                            <span className="text-sm text-gray-900">{t.entry_trans_type || "-"}</span>
                          </div>
                          ...
                          <div className="flex items-center justify-between">
                            ...
                            <span className="text-sm font-medium text-gray-900">{t.parking_dur || "-"} mins</span>
                          </div>
                          ...
                          <div className="p-3 bg-blue-50 rounded-lg border border-blue-200">
                            <div className="space-y-2">
                              <div className="flex items-center justify-between">
                                ...
                                <span className="text-sm font-medium text-blue-900">{t.card_type || "-"}</span>
                              </div>
                              <div className="flex items-center justify-between">
                                <span className="text-xs text-blue-700">Card Number</span>
                                <span className="font-mono text-sm text-blue-900">{t.card_number || "-"}</span>
                              </div>
                            </div>
                          </div>
            ...
            <div className="bg-white/80 backdrop-blur-xl border border-white/20 rounded-xl shadow-lg">
              <div className="flex flex-col items-center justify-center p-12 text-center">
                ...
                <h3 className="font-semibold text-gray-900 mb-2">No Outstanding Transactions Found</h3>
                <p className="text-sm text-gray-600 max-w-md">
                  Select a report date and click "Search" to view outstanding movement transaction records.
                </p>
              </div>
            </div>
          )}
              ...
              <div>
                ...
                <ul className="space-y-1.5 text-sm text-blue-800">
                  <li className="flex items-start gap-2">
                    ...
                    <span>Select a specific date to view all outstanding vehicle movement transactions</span>
                  </li>
                  <li className="flex items-start gap-2">
                    <span className="w-1 h-1 rounded-full bg-blue-600 mt-2 flex-shrink-0"></span>
                    <span>Outstanding transactions show vehicles that have not yet exited the parking facility</span>
                  </li>
                  <li className="flex items-start gap-2">
                    <span className="w-1 h-1 rounded-full bg-blue-600 mt-2 flex-shrink-0"></span>
                    <span>The report includes entry time, parking duration, fees, and card information</span>
                  </li>
                </ul>
              </div>
            </div>
