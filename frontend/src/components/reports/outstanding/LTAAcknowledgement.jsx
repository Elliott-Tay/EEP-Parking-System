  ...
  const handleSearch = async () => {
    ...
    try {
      ...
      if (data && data.length > 0) {
        toast.success(`Found ${data.length} LTA acknowledgement file${data.length !== 1 ? 's' : ''}`);
      } else {
        toast.info("No LTA acknowledgement files found");
      }
    } catch (err) {
      console.error(err);
      setRecords([]);
      toast.error("Failed to fetch LTA acknowledgement files");
    } finally {
      setLoading(false);
    }
  };
        ...
        <div className="max-w-7xl mx-auto">
          ...
          <div className="mb-6">
            <div className="flex items-center gap-3 mb-2">
              ...
              <div>
                <h1 className="text-3xl font-bold text-gray-900">Outstanding LTA Acknowledge Files</h1>
                <p className="text-sm text-gray-600 mt-1">
                  View and manage LTA acknowledgement files with settlement information
                </p>
              </div>
            </div>
          </div>
          ...
          <div className="bg-white/80 backdrop-blur-xl border border-white/20 rounded-xl shadow-lg mb-6">
            <div className="bg-gradient-to-r from-blue-50 to-blue-100/50 border-b px-6 py-4 rounded-t-xl">
              ...
              <p className="text-sm text-blue-700 mt-1">Filter LTA acknowledgement files by settle date (optional)</p>
            </div>
              ...
              <div className="space-y-6">
                ...
                <div className="space-y-2">
                  ...
                  <p className="text-xs text-gray-500 flex items-center gap-1">
                    ...
                    Leave empty to view all LTA acknowledgement files, or select a specific date to filter
                  </p>
                </div>
          ...
          {loading ? (
            <div className="bg-white/80 backdrop-blur-xl border border-white/20 rounded-xl shadow-lg p-12">
              <div className="flex flex-col items-center justify-center gap-4">
                ...
                <p className="text-gray-600 font-medium">Loading LTA acknowledgement files...</p>
                ...
                <div className="bg-gradient-to-r from-green-50 to-green-100/50 border-b px-6 py-4 rounded-t-xl">
                  <div className="flex items-center justify-between">
                    <div className="flex items-center gap-2">
                      ...
                      <h3 className="font-semibold text-gray-900">LTA Acknowledgement File Records</h3>
                    </div>
                  ...
                  <p className="text-sm text-green-700 mt-1">
                    {fileDate ? `LTA acknowledgement files for ${fileDate}` : 'All LTA acknowledgement files'}
                  </p>
                </div>
                      ...
                      <tbody className="divide-y divide-gray-200">
                        {records.map((r, idx) => (
                          <tr key={r.id} className="hover:bg-blue-50/50 transition-colors duration-150">
                            ...
                            <td className="px-4 py-3">
                              <span className="text-sm font-medium text-gray-900">{formatNumber(r.total_transaction)}</span>
                            </td>
                            <td className="px-4 py-3">
            ...
            <div className="bg-white/80 backdrop-blur-xl border border-white/20 rounded-xl shadow-lg">
              <div className="flex flex-col items-center justify-center p-12 text-center">
                ...
                <h3 className="font-semibold text-gray-900 mb-2">No LTA Acknowledgement Files Found</h3>
                <p className="text-sm text-gray-600 max-w-md">
                  {fileDate 
                    ? `No LTA acknowledgement files found for ${fileDate}. Try a different date or clear the filter.`
                    : "Click the Search button to load LTA acknowledgement files."}
                </p>
              </div>
            </div>
          )}
          ...
          <div className="mt-6 bg-gradient-to-br from-blue-50 to-blue-100/50 border border-blue-200 rounded-xl p-5">
            <div className="flex items-start gap-3">
              ...
              <div>
                <h4 className="font-semibold text-blue-900 mb-2">LTA Acknowledgement File Information</h4>
                <ul className="space-y-1.5 text-sm text-blue-800">
                  <li className="flex items-start gap-2">
                    <span className="w-1 h-1 rounded-full bg-blue-600 mt-2 flex-shrink-0"></span>
                    <span>LTA (Land Transport Authority) acknowledgement files confirm receipt of outstanding transaction submissions</span>
                  </li>
                  <li className="flex items-start gap-2">
                    <span className="w-1 h-1 rounded-full bg-blue-600 mt-2 flex-shrink-0"></span>
                    <span>You can filter by settle date or view all acknowledgement files by leaving the date field empty</span>
                  </li>
                  <li className="flex items-start gap-2">
                    <span className="w-1 h-1 rounded-full bg-blue-600 mt-2 flex-shrink-0"></span>
                    <span>Each file shows the total transactions, total amount, settlement date, and send time</span>
                  </li>
                  <li className="flex items-start gap-2">
                    <span className="w-1 h-1 rounded-full bg-blue-600 mt-2 flex-shrink-0"></span>
                    <span>Track when acknowledgements were sent to monitor the status of your transaction submissions</span>
                  </li>
                </ul>
              </div>
            </div>
