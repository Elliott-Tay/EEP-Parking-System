...
export default function MovementTransaction() {
  ...
  const handleSearch = async () => {
    try {
      ...
      setResults(data); // assuming API returns an array of movements
    } catch (error) {
      console.error(error);
  ...
  return (
    <div className="min-h-screen bg-gray-50 flex flex-col items-center justify-start p-8">
      <h1 className="text-2xl font-bold mb-6">Movement Transaction Enquiry</h1>
              ...
              <tbody>
                {results.map((item, idx) => (
                  <tr key={idx} className="hover:bg-gray-50">
                    ...
                    <td className="border px-3 py-2">{item.vehicle_no}</td>
                    <td className="border px-3 py-2">{item.timestamp}</td>
                    <td className="border px-3 py-2">{item.details}</td>
                  </tr>
                ))}
              </tbody>
            </table>
