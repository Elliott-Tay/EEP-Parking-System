...
export default function ExitInvalidTransactionDetail() {
  ...
  const [chuDebitCode, setChuDebitCode] = useState("");
  ...
  const handleSearch = async () => {
    try {
      const queryParams = new URLSearchParams({
        ...
        end_date: endDate,
        chu_debit_code: chuDebitCode,
        iu_ticket_no: iuTicketNo,
      });

      const response = await fetch(
        `${process.env.REACT_APP_BACKEND_API_URL}/api/exit-invalid-transactions?${queryParams.toString()}`,
      ...
      setResults(data); // assuming API returns an array of invalid exit transactions
    } catch (error) {
      console.error(error);
  ...
  return (
    <div className="min-h-screen bg-gray-50 flex flex-col items-center justify-start p-8">
      ...
      <div className="w-full max-w-6xl bg-white border rounded-lg shadow p-6 space-y-4">
        ...
        {/* CHU Debit Code */}
        <div className="flex gap-4 items-center">
          <label className="font-medium text-gray-700 w-32">CHU Debit Code:</label>
          <input
            type="text"
            value={chuDebitCode}
            onChange={(e) => setChuDebitCode(e.target.value)}
            placeholder="Enter CHU Debit Code"
