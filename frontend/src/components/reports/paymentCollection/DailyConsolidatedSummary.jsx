...
function DailyConsolidatedSummary() {
    ...
    const fetchData = async () => {
      ...
      try {
        ...
        const formatted = data.map((d, i) => ({
          id: i + 1,
          date: d.log_date.split("T")[0],
          totalPayments: d.totalPayments,
          totalTransactions: d.totalTransactions,
          discrepancies: d.discrepancies,
        }));
        setRecords(formatted);
        toast.success(`Loaded ${formatted.length} consolidated summar${formatted.length !== 1 ? 'ies' : 'y'}`);
      } catch (err) {
        console.error(err);
  ...
  const filteredRecords = records.filter((r) =>
    [r.date, r.totalPayments, r.totalTransactions, r.discrepancies].some((val) =>
  ...
  const handleDownload = (record) => {
    ...
    toast.success(`Downloaded summary for ${record.date}`);
  };

  // Calculate statistics
  const totalRecords = filteredRecords.length;
  const totalPaymentsSum = filteredRecords.reduce((sum, r) => sum + r.totalPayments, 0);
  const totalTransactionsSum = filteredRecords.reduce((sum, r) => sum + r.totalTransactions, 0);
  const totalDiscrepancies = filteredRecords.filter(r => r.discrepancies !== 0).length;
        ...
        <div className="max-w-7xl mx-auto">
          ...
          <div className="mb-6">
            <div className="flex items-center gap-3 mb-2">
              ...
              <div>
                <h1 className="text-3xl font-bold text-gray-900">Daily Consolidated Summary</h1>
                <p className="text-sm text-gray-600 mt-1">
                  Comprehensive daily financial reconciliation and transaction summary
                </p>
              </div>
            </div>
          </div>
          ...
          {loading ? (
            <div className="bg-white/80 backdrop-blur-xl border border-white/20 rounded-xl shadow-lg p-12">
              <div className="flex flex-col items-center justify-center gap-4">
                ...
                <p className="text-gray-600 font-medium">Loading consolidated summaries...</p>
                ...
                <div className="bg-white/80 backdrop-blur-xl border border-white/20 rounded-xl shadow-lg p-5">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      ...
                      <p className="text-2xl font-bold text-gray-900">${totalPaymentsSum.toLocaleString()}</p>
                    </div>
                  </div>
                </div>
                ...
                <div className="bg-white/80 backdrop-blur-xl border border-white/20 rounded-xl shadow-lg p-5">
                  <div className="flex items-center gap-3">
                    <div className={`w-12 h-12 rounded-lg flex items-center justify-center ${
                      totalDiscrepancies > 0 ? 'bg-red-100' : 'bg-green-100'
                    }`}>
                    ...
                    <div>
                      ...
                      <p className={`text-2xl font-bold ${
                        totalDiscrepancies > 0 ? 'text-red-600' : 'text-green-600'
                      }`}>{totalDiscrepancies}</p>
                    </div>
                  </div>
                </div>
              </div>
                ...
                <div className="bg-gradient-to-r from-purple-50 to-purple-100/50 border-b px-6 py-4 rounded-t-xl">
                  <div className="flex items-center justify-between">
                    <div className="flex items-center gap-2">
                      ...
                      <h3 className="font-semibold text-gray-900">Consolidated Summary Records</h3>
                    </div>
                  ...
                  <p className="text-sm text-purple-700 mt-1">
                    {searchTerm ? `Filtered by "${searchTerm}"` : 'All daily consolidated summaries'}
                  </p>
                </div>
          ...
          <div className="mt-6 bg-gradient-to-br from-blue-50 to-blue-100/50 border border-blue-200 rounded-xl p-5">
            <div className="flex items-start gap-3">
              ...
              <div>
                <h4 className="font-semibold text-blue-900 mb-2">Daily Consolidated Summary Information</h4>
                <ul className="space-y-1.5 text-sm text-blue-800">
                  <li className="flex items-start gap-2">
                    <span className="w-1 h-1 rounded-full bg-blue-600 mt-2 flex-shrink-0"></span>
                    <span>View comprehensive daily summaries consolidating all payment and transaction data</span>
                  </li>
                  <li className="flex items-start gap-2">
                    <span className="w-1 h-1 rounded-full bg-blue-600 mt-2 flex-shrink-0"></span>
                    <span>Use the search box to filter records by date, payments, transactions, or discrepancies</span>
                  </li>
                  <li className="flex items-start gap-2">
                    <span className="w-1 h-1 rounded-full bg-blue-600 mt-2 flex-shrink-0"></span>
                    <span>Discrepancies highlighted in red indicate mismatches requiring investigation and reconciliation</span>
                  </li>
                  <li className="flex items-start gap-2">
                    <span className="w-1 h-1 rounded-full bg-blue-600 mt-2 flex-shrink-0"></span>
                    <span>Click "Preview" to view detailed information or "Download" to export individual daily summaries as CSV</span>
                  </li>
                  <li className="flex items-start gap-2">
                    <span className="w-1 h-1 rounded-full bg-blue-600 mt-2 flex-shrink-0"></span>
                    <span>Summary cards provide quick insights: total days, aggregate payments, transactions, and discrepancy counts</span>
                  </li>
                </ul>
              </div>
            </div>
...
function TableRow({ record, onPreview, onDownload }) {
  return (
    <tr className="hover:bg-blue-50/50 transition-colors duration-150">
      ...
      <td className="px-4 py-3 text-sm font-mono font-medium text-gray-900">
        ${record.totalPayments.toLocaleString()}
      </td>
      ...
      <td className={`px-4 py-3 text-sm ${record.discrepancies !== 0 ? "text-red-600 font-semibold" : "text-green-600"}`}>
        <span className={`inline-flex items-center rounded-full px-2.5 py-0.5 text-xs font-medium border ${
          record.discrepancies !== 0
            ? "bg-red-100 text-red-800 border-red-200"
            : "bg-green-100 text-green-800 border-green-200"
        }`}>
          {record.discrepancies !== 0 && <AlertTriangle className="w-3 h-3 mr-1" />}
          {record.discrepancies === 0 && <CheckCircle2 className="w-3 h-3 mr-1" />}
...
function MobileCard({ record, onPreview, onDownload }) {
  return (
    <div className="p-4 bg-gradient-to-br from-white to-gray-50 rounded-lg border border-gray-200 shadow-sm">
      <div className="space-y-3">
        ...
        <div className="grid grid-cols-2 gap-3">
          <div>
            ...
            <p className="text-sm font-mono font-medium text-gray-900">${record.totalPayments.toLocaleString()}</p>
          </div>
...
function PreviewModal({ record, onClose }) {
  return (
    <div className="fixed inset-0 bg-black/50 backdrop-blur-sm flex justify-center items-center z-50 p-4 animate-in fade-in-0 duration-200">
      <div className="bg-white rounded-xl shadow-2xl w-full max-w-md relative animate-in zoom-in-95 duration-200">
          ...
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-2">
              ...
              <div>
                ...
                <p className="text-sm text-purple-700">{record.date}</p>
              </div>
            </div>
            <button 
              onClick={onClose} 
              className="w-8 h-8 rounded-lg hover:bg-purple-200 flex items-center justify-center text-gray-500 hover:text-gray-700 transition-colors"
            >
              <X className="w-5 h-5" />
            </button>
          </div>
        ...
        <div className="p-6 space-y-4">
          <div className="p-4 bg-gradient-to-br from-green-50 to-green-100/50 rounded-lg border border-green-200">
            <div className="flex items-center gap-2 mb-2">
              ...
              <span className="text-sm font-medium text-green-900">Total Payments</span>
            </div>
          ...
          <div className={`p-4 rounded-lg border ${
            ...
            <div className="flex items-center gap-2 mb-2">
              ...
              <span className={`text-sm font-medium ${
                record.discrepancies !== 0 ? "text-red-900" : "text-green-900"
              }`}>
                Discrepancies
              </span>
            </div>
            <p className={`text-2xl font-bold ${
              record.discrepancies !== 0 ? "text-red-700" : "text-green-700"
            }`}>
              {record.discrepancies}
            </p>
            {record.discrepancies !== 0 && (
              <p className="text-xs text-red-600 mt-2">
                Attention required: This record has discrepancies that need reconciliation
              </p>
            )}
          </div>
        </div>
        ...
        <div className="px-6 pb-6">
          <button
            onClick={onClose}
            className="w-full px-4 py-2 bg-gradient-to-r from-red-600 to-red-700 text-white rounded-lg hover:from-red-700 hover:to-red-800 shadow-lg transition-all duration-200 font-semibold"
          >
            Close
          </button>
        </div>
