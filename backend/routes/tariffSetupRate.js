...
// setup the new tariff (flat payload)
router.post("/tariff-setup", async (req, res) => {
  ...
  if (!vehicle_type || !day_of_week || !from_time || !to_time) {
    return res.status(400).json({ error: "vehicle_type, day_of_week, from_time, and to_time are required" });
  }

  // default effective dates if not provided
  const effectiveStartStr = effectiveStart || new Date().toISOString().split("T")[0];
  const effectiveEndStr = effectiveEnd || "2099-12-31";
  ...
  try {
    ...
    await new sql.Request(transaction)
      ...
      .input("from_time", sql.VarChar, from_time)
      .input("to_time", sql.VarChar, to_time)
      .input("rate_type", sql.NVarChar, rate_type || "Hourly")
      .input("every", sql.Int, every || null)
      .input("min_fee", sql.Money, truncate2(min_fee))
      .input("grace_time", sql.Int, grace_time || null)
      .input("first_min_fee", sql.Money, truncate2(first_min_fee))
      .input("min_charge", sql.Money, truncate2(min_charge))
      .input("max_charge", sql.Money, truncate2(max_charge))
      .input("effective_start", sql.VarChar, effectiveStartStr)
      .input("effective_end", sql.VarChar, effectiveEndStr)
      .query(`
...
router.get("/tariff-setup", async (req, res) => {
  const { vehicleType, rateType } = req.query;
  ...
  try {
    ...
    if (vehicleType) {
      ...
      request.input("vehicle_type", sql.NVarChar, vehicleType);
    }

    if (rateType) {
      conditions.push("rate_type = @rate_type");
      request.input("rate_type", sql.NVarChar, rateType);
    }
    ...
    // Format response
    const response = {
      effectiveStart: filtered[0].effective_start,
      effectiveEnd: filtered[0].effective_end,
    };

    // Group rates by day_of_week (optionally also by rate_type)
    const groupedRates = {};
