...
    const createComputer = (exit, vehicleType = vehicleTypeString) => {
...
    // --- NEW TEST CASES ADDED (Focus on minimum charge and mid-range rounding boundaries) ---

    // 29. Minimum Fee Boundary Down: Stay 14 seconds over grace. Fee should round down to $0.00.
    test('29. Stay of 60m 14s should cost $0.00 (just under $0.01 threshold)', () => {
        const exit = `${testDate}T11:00:14Z`; 
        const computer = createComputer(exit);
        // Chargeable minutes = 14/60 ≈ 0.2333... mins. Fee = 0.2333... * 0.02 ≈ 0.00466... -> 0.00
        expect(computer.computeParkingFee()).toBeCloseTo(0.00, 2);
    });

    // 30. Minimum Fee Boundary Up: Stay 15 seconds over grace. Fee should reach $0.01 (0.005).
    test('30. Stay of 60m 15s should cost $0.01 (exactly $0.01 threshold)', () => {
        const exit = `${testDate}T11:00:15Z`; 
        const computer = createComputer(exit);
        // Chargeable minutes = 15/60 = 0.25 mins. Fee = 0.25 * 0.02 = 0.005 -> 0.01
        expect(computer.computeParkingFee()).toBeCloseTo(0.01, 2);
    });

    // 31. Mid-Range Rounding Down: 3h 46m 44s total. Fee should round down from $3.334 to $3.33.
    test('31. Stay of 3h 46m 44s should cost $3.33 (just under round-up threshold)', () => {
        const exit = `${testDate}T13:46:44Z`;
        const computer = createComputer(exit);
        // Total mins: 226m 44s. Chargeable mins: 166m 44s ≈ 166.7333...
        // Fee = 166.7333... * 0.02 ≈ 3.33466... -> 3.33
        expect(computer.computeParkingFee()).toBeCloseTo(3.33, 2);
    });

    // 32. Mid-Range Rounding Up: 3h 46m 45s total. Fee should round up from $3.335 to $3.34.
    test('32. Stay of 3h 46m 45s should cost $3.34 (exactly the round-up threshold)', () => {
        const exit = `${testDate}T13:46:45Z`; 
        const computer = createComputer(exit);
        // Total mins: 226m 45s. Chargeable mins: 166m 45s = 166.75
        // Fee = 166.75 * 0.02 = 3.335 -> 3.34
        expect(computer.computeParkingFee()).toBeCloseTo(3.34, 2);
    });


    // --- EXISTING NEW TEST CASES (Renumbered) ---

    // 33. Boundary Check: 24 hours and 1 second overage.
    test('33. Stay of 24 hours and 1 second should cost $27.60 (boundary precision)', () => {
        const exit = `2025-10-28T10:00:01Z`; 
        const computer = createComputer(exit);
        // Total minutes = (24 * 60) + (1 / 60) = 1440.01666...
        // Chargeable minutes = 1380.01666...
        // Fee = 1380.01666... * 0.02 = 27.60033... -> $27.60
        expect(computer.computeParkingFee()).toBeCloseTo(27.60, 2);
    });

    // 34. Very Long Stay Boundary Check: 7 days and 1 second overage.
    test('34. Stay of 7 days and 1 second should cost $200.40 (long-term boundary precision)', () => {
        const exit = `2025-11-03T10:00:01Z`; 
        const computer = createComputer(exit);
        // Total minutes = (7 * 1440) + (1 / 60) = 10080.01666...
        // Chargeable minutes = 10020.01666...
        // Fee = 10020.01666... * 0.02 = 200.40033... -> $200.40
        expect(computer.computeParkingFee()).toBeCloseTo(200.40, 2);
    });
